window.addEventListener('DOMContentLoaded', () => {
    ...
    function draw() {
        ...
        drawProfileImage(); drawWatermark(intensity);
    }
    ...
    const drawFuncs = {
        ...
        shatter: (c, o=0) => { const l=frequencyData.length, hX=canvas.width/2+o, hY=canvas.height/2; const avg=getAverageVolume(frequencyData)*(settings.sensitivity/1.5); const C=c||getDrawColor(0.5); settings.filledShapes?ctx.fillStyle=C:ctx.strokeStyle=C; ctx.lineWidth=3; ctx.beginPath(); 
            // Draw base shape
            for(let i=1;i<l;i++){const a=(i/l)*2*Math.PI, r=100+Math.max(0, avg), x=hX+Math.cos(a)*r, y=hY+Math.sin(a)*r; i===1?ctx.moveTo(x,y):ctx.lineTo(x,y);} 
            ctx.closePath(); settings.filledShapes?ctx.fill():ctx.stroke(); 
            // Draw "Shards" instead of particles
            if(avg > 50) {
                const shardCount = 20;
                for(let i=0; i<shardCount; i++) {
                    const angle = (i/shardCount) * Math.PI*2 + dynamicRotation;
                    const rBase = 100 + avg;
                    const rTip = rBase + (frequencyData[i*5] * (settings.sensitivity/2));
                    ctx.beginPath();
                    ctx.moveTo(hX + Math.cos(angle - 0.1)*rBase, hY + Math.sin(angle - 0.1)*rBase);
                    ctx.lineTo(hX + Math.cos(angle)*rTip, hY + Math.sin(angle)*rTip);
                    ctx.lineTo(hX + Math.cos(angle + 0.1)*rBase, hY + Math.sin(angle + 0.1)*rBase);
                    ctx.fill();
                }
            }
        },
        ...
        circularWaveform: (c, o=0) => { const l=timeDomainData.length, hX=canvas.width/2+o, hY=canvas.height/2, C=c||settings.gradientColors[0]; ctx.strokeStyle=settings.rainbowMode ? getDrawColor(0.5) : (c ? c : ctx.createLinearGradient(0,0,canvas.width,canvas.height)); if(!c && !settings.rainbowMode){ settings.gradientColors.forEach((col,i)=>ctx.strokeStyle.addColorStop(i/(settings.gradientColors.length-1), col)); } ctx.lineWidth=3; ctx.beginPath(); for(let i=0;i<l;i++){ const d=(timeDomainData[i]-128), r=200+(d*(settings.sensitivity/1.5)), a=(i/l)*2*Math.PI, x=hX+Math.cos(a)*r, y=hY+Math.sin(a)*r; i===0?ctx.moveTo(x,y):ctx.lineTo(x,y); } ctx.closePath(); ctx.stroke();},
        ...
        // --- NEW VISUALIZERS ---
        barcode: (c, o=0) => { const l=frequencyData.length; const w=canvas.width/l * 4; for(let i=0; i<l; i+=4) { const H=frequencyData[i]*settings.sensitivity; const alpha = H/255; ctx.fillStyle = c || getDrawColor(i/l); ctx.globalAlpha = alpha; ctx.fillRect(i*(canvas.width/l) + o, (canvas.height/2) - (canvas.height/4), w, canvas.height/2); } ctx.globalAlpha = 1.0; },
        spiral: (c, o=0) => { const cx=canvas.width/2+o, cy=canvas.height/2; ctx.lineWidth=3; ctx.beginPath(); for(let i=0; i<frequencyData.length; i++) { const angle = 0.1 * i + dynamicRotation; const radius = 5 * i * 0.05 + (frequencyData[i] * settings.sensitivity/3); const x = cx + Math.cos(angle)*radius; const y = cy + Math.sin(angle)*radius; ctx.strokeStyle = c || getDrawColor(i/frequencyData.length); if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); } ctx.stroke(); },
        river: (c, o=0) => { const cx=canvas.width/2+o, cy=canvas.height/2; const l=frequencyData.length; ctx.lineWidth=2; for(let i=0; i<50; i++) { ctx.beginPath(); ctx.strokeStyle = c || getDrawColor(i/50); const yBase = cy - 200 + (i*10); for(let x=0; x<canvas.width; x+=20) { const idx = Math.floor((x/canvas.width)*l); const offset = Math.sin(x*0.01 + dynamicRotation + i*0.2) * (frequencyData[idx]*settings.sensitivity/5); if(x===0) ctx.moveTo(x+o, yBase+offset); else ctx.lineTo(x+o, yBase+offset); } ctx.stroke(); } }
    };
    ...
    function updateContrast() {
        ...
        uiBtns.forEach(btn => { btn.style.color = yiq >= 128 ? '#000' : '#fff'; btn.querySelector('svg').style.fill = yiq >= 128 ? '#000' : '#fff'; });
    }
    ...
    async function initializeAndStartLiveVisualizer() { settingsButton.classList.add('visible'); await startOrUpdateAudioSource(); await populateMicrophoneList(); updateAudioSourceUI(); handleAccentCheck(); updateGradientUI(); handleVisualizerOptionsVisibility(); handleRainbowModeVisibility(); }
    ...
    initializeAndStartLiveVisualizer();
    draw();
});